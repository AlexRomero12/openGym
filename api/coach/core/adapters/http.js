...
export function httpAdapter(spec) {
  ...
  const adapter = {
     ...
     */
    async invoke(opts = {}) {
      const { cfg, prompt, system, schema, env, model, effort, timeoutMs = DEFAULT_TIMEOUT_MS, fetch: fetchImpl = globalThis.fetch, signal, headers: extraHeaders } = opts;
      const base = adapter.baseUrl(cfg);
      ...
      let body = spec.body({ model: chosen, prompt, system: system || null, schema: schema || null, maxTokens: MAX_OUTPUT_TOKENS, effort: effort || null });
