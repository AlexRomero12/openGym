...
// rather than "someone tried a passkey we don't know". Turning those into sentences
...
//
// The fork translates this surface with the rest of the app (upstream kept the admin screens
// English-only). The English sentence is the lookup key, so `t()` on the way out is what makes
// the log follow the UI language; times already follow it through dateLocale.
...
export const auditLabel = ev => (LABELS[ev] ? t(LABELS[ev]) : String(ev || t('Unknown event')))
...
export const auditReason = msg => (REASONS[msg] ? t(REASONS[msg]) : (msg ? String(msg) : ''))
...
export function auditLine(e) {
  ...
  if (e.tname) parts.push(t('→ {0}', e.tname))
...
export function fmtWhen(ts, now = Date.now()) {
  ...
  if (sameDay) return t('today {0}', time)
