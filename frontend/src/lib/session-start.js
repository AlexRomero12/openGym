...
/**
 * The Coach's confirmed load, as the session sees it: work rows only, warm-ups left for the
 * reramp below, a per-side row keeping its two sides in sync. `done` rows are never touched —
 * an override that arrives mid-session (someone accepts a proposal while the workout is open)
 * must not rewrite a set that has already been performed.
 */
function applyCoachWeight(sets, w) {
  return sets.map(s => {
    if (s.done || isWarmupRow(s)) return s
    if (isSideSet(s)) return syncSideAggregate({ ...s, sides: { L: { ...s.sides.L, w }, R: { ...s.sides.R, w } } })
    return { ...s, w }
  })
}
 ...
 *
 * `opts.prefill` is `S.prefill[iso].ex` for the session's date: weights the lifter confirmed
 * from a Coach load estimate. They win over what the engine would load — but only for this
 * build, and only before the intensifier plan, so a drop-set's chain is computed from the
 * weight actually on the bar. Nothing is written back into the plan or the history.
 */
export function buildSessionEntries(st, r, opts = {}) {
